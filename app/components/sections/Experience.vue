      ...
      <div class="relative min-h-[500px]">
        ...
        <Transition name="fade" mode="out-in">
          ...
          <div v-else-if="activeTab === 'education'" key="education" class="space-y-8">
            <div v-for="school in educationHistory" :key="school.id" 
                 class="relative pl-8 border-l-2 border-slate-200 dark:border-navy-700 hover:border-indigo-500 dark:hover:border-indigo-500 transition-colors duration-300">
              
              <div class="absolute -left-[9px] top-0 w-4 h-4 rounded-full bg-indigo-600 ring-4 ring-white dark:ring-navy-900"></div>

              <div class="mb-2">
                <h3 class="text-2xl font-bold text-navy-900 dark:text-white">{{ school.name }}</h3>
              </div>

              <div v-for="role in school.roles" :key="role.id" class="mb-6 last:mb-0 bg-white dark:bg-navy-800 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-navy-700/50 hover:shadow-lg transition-shadow border-t-4 border-t-indigo-500/20">
                <div class="flex flex-col md:flex-row md:justify-between md:items-start gap-2 mb-3">
                  <h4 class="text-lg font-bold text-indigo-600 dark:text-indigo-400">{{ role.title }}</h4>
                  <span class="text-sm font-mono text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-navy-900 px-2 py-1 rounded">
                    {{ role.period }}
                  </span>
                </div>
                <p class="text-slate-600 dark:text-mist-200 mb-4 leading-relaxed whitespace-pre-line">
                  {{ role.description }}
                </p>
                <div class="flex flex-wrap gap-2">
                  <span v-for="tech in role.techStack" :key="tech" 
